...
async function downloadPackages(packages, options = {}) {
    ...
    const result = await map(packagesToDownload, ({ name, version, isLatest }) => downloadPackageTarball(name, version, { destFolder, isLatest }).catch(catchError))
...
function downloadPackageTarball(name, version = 'latest', options = { destFolder: '.' }) {
    ...
    return new Promise((resolve, reject) => {
        const stream = pacote.tarball.stream(`${name}@${version}`, { cache: pacoteCacheFolder });
        ...
        // Create tgz file on finish
        stream.on('finish', () => {
            stream.pipe(createWriteStream(`${destFolder}/${name.replace('/', '-')}-${version}${isLatest ? '-latest' : ''}.tgz`))
                .on('error', (err) => {
                    reject(err);
                })
                .on('finish', () => {
...
async function resolveDependencies(manifest, options) {
    ...
    const result = await filter(dependencies, ({ name, version }) => !resolvedPackages.get(name, version))
        .map(({ name, version }) => getPackageManifest(name, version).catch(catchError))
